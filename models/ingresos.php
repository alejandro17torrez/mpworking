<?php
    ...
    Class Ingresos
    {
        ...
        $filas, $dato, $debe, $haber, $date, $asiento, $numero, $comprobante, $documento, $concepto, $signo, $grupo;
        ...
        public function movimientos_automaticos_recibos_ingresos($caja, $banco, $cuenta, $IVA, $Retenciones)
        {
            // este va!! inserta la cuenta hija ligada al concepto
            $this->sql = "INSERT INTO movimientos (comprobante_id, cuenta_id, parcial) 
            SELECT ri.comprobante_diario, ri.concepto, ri.cantidad FROM recibos_ingreso ri ORDER BY id DESC LIMIT 1";
            $this->execute = $this->enlace->prepare($this->sql);
            $this->execute->execute();
            ...
            echo "<br>";
            echo "Movimientos";
            //Obtener el parcial y id del concepto del último movimiento
            ...
            $this->sql = "SELECT caa.codigo, cc.signo, gr.grupo FROM recibos_ingreso ri, cuentas_conceptos  cc, cuentas_cooperativas coop, catalogo caa, grupos gr WHERE ri.concepto = cc.id AND cc.cuentas_coop = coop.id AND coop.cuenta = caa.id AND gr.id = caa.grupo ORDER BY ri.id DESC LIMIT 1 ";
            $this->execute = $this->enlace->prepare($this->sql);
            $this->execute->execute();
            $this->filas = $this->execute->fetchAll(PDO::FETCH_ASSOC);
            ...
            $this->sql = "SELECT ri.comprobante_diario, ri.cantidad FROM recibos_ingreso ri ORDER BY id DESC LIMIT 1";
            $this->execute = $this->enlace->prepare($this->sql);             
            $this->execute->execute($this->data);
            $this->filas = $this->execute->fetchAll(PDO::FETCH_ASSOC);
            ...
            */
             echo "<br>";
             echo "El grupo que se ocupa es: ". $this->grupo;

            if (($this->grupo == "ingresos" || $this->grupo == "capital" || $this->grupo == "pasivos") && $caja == "110101" && $banco == null && $cuenta == null && $this->signo == "-") {
               ...
               // echo "<br>";
                ...
                //insertar al padre
                $this->sql = "INSERT INTO movimientos (comprobante_id, cuenta_id, debe) VALUES (:compro, :cuentaid, :cant)";
                $this->execute = $this->enlace->prepare($this->sql);
                $this->execute->execute($this->data);
            ...
            elseif (($this->grupo == "ingresos" || $this->grupo == "capital" || $this->grupo == "pasivos") && $caja == null && $banco == "1102" && $cuenta != null && $this->signo == "-") {
                ...
                echo "Entré trabajando con el banco (borrar despues) " .$cuenta;
                echo "<br>";
            ...
            if ($this->grupo == "activos" && $caja == "110101" && $banco == null && $cuenta == null && $this->signo == "+") {
            ...
            elseif ($this->grupo == "activos" && $caja == null && $banco == "1102" && $cuenta != null && $this->signo == "+") {
        ...
        public function insertar_recibo_ingreso_con_comprobante_diario(
            ...
            $vIdConcepto,$vCaja_O_banco,$vNumeroBaucher,$vLugar,$vFecha,$vIdCooperativa, $vCuenta_banco)
        {
            ...
            $this->sql = "INSERT INTO `multipro`.`recibos_ingreso` 
            ...
            if ($vCaja_O_banco == "110101") {
                $this->movimientos_automaticos_recibos_ingresos("110101", null, null, null, null);
            }
            elseif ($vCaja_O_banco == "1102" && $vCuenta_banco != null) {
                $this->movimientos_automaticos_recibos_ingresos(null, $vCaja_O_banco, $vCuenta_banco, null, null);
            }
    ...
    //$objeto= new Ingresos();
    //$objeto->insertar_recibo_ingreso_con_comprobante_diario(
       // '1','TERCERO','ALEJANDRO PALACIOS','C$','1000','Mil Cordobas Netos','55','1102',null,
        //'MATAGALPA,Matagalpa','2019-09-11','81', '1013467A');
    //$objeto->test();
